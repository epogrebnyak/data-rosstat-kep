...
def test_locate_additional_specs_file_exists():
    additional_specfiles = ['__spec_budget_expense.txt', '__spec_budget_revenue.txt', '__spec_budget_surplus.txt', '__spec_cpi.txt', '__spec_foreign_trade.txt', '__spec_invest_src.txt', '__spec_overdue.txt', '__spec_profit.txt', '__spec_receivable.txt', '__spec_retail.txt']
    for f in ini.__locate_additional_specs__(ini.get_default_spec_folder()):
        assert f.name in additional_specfiles
        additional_specfiles.remove(f.name)
    assert additional_specfiles == []
    
def test_max_year_returns_2017_or_greater(): 
    assert ini.max_year() >= 2017    
