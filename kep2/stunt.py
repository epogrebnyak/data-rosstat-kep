...
# TODO: use fucntions below for csv and yaml access 
#
#def get_csv_data(content):
#    
#     # get csv data  
#     with Tempfile(content_string = content) as path:
#         return list(CSV_Reader(path).yield_dicts())		
#    
...
class TestDatapoints1(unittest.TestCase):
    
    # EP:use setUp() and tearDown() methods here?
    ...
    def test_dataframes(self):
        ...
        # get test parsing definitions
        with Tempfile(content_string = yaml_content) as path:
            pi = ParsingDefinition(path)
    		
    	 # get csv data     
        with Tempfile(content_string = input_csv) as path:
