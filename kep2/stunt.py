...
from parsing_definitions import ParsingDefinition, Tempfile
...
# -----------------------------------------------------------------------------
#
# Suit 1. Test Datapoints class
#
# -----------------------------------------------------------------------------

# Input csv data for creating Datapoints object
input_1 = """
...
#Concatenate csv blocks 
input_csv = input_1 + input_2

# Input parsing instructions for creating Datapoints object
YAML = """
# Раздел 1. Информация по сегментам
# Section 1. Segment information 
# segment information
...
# Testing Datapoints class in datapoints.py
def test_datapoints():
	# DR: Get test parsing definitions
    with Tempfile(yaml_string = YAML) as path:
        pi = ParsingDefinition(path)
		
	# DR: Get test data	points
    with Tempfile(yaml_string = input_csv) as path:
        csv_dicts = list(CSV_Reader(path).yield_dicts())		
	...
	# DR: Get result data for the assertion	
...
if __name__ == '__main__':
    ...
    # Testing Datapoints class from datapoints.py
    test_datapoints()	
