...
from .csv_data import doc_to_lists, row_as_dict, csv_file_to_dicts, yield_dicts_from_string, CSV_Reader
...
class Test_row_as_dict(unittest.TestCase):
    ...
    def test_datarow_head_and_data(self):
        assert row_as_dict(self.row)['head'] == '2013'
        assert row_as_dict(self.row)['data'] == ['10', '20', '30', '40']

    def test_textrow_head(self):
        assert row_as_dict(['1. Сводные показатели', '', ''])['head'] == '1. Сводные показатели'
...
class TestCSV_yield_from_string(Match_CSV_Content):
    def test_yield_from_string(self):
        assert list(yield_dicts_from_string(self._csv_content)) == self._dicts
...
class TestCSV_Reader(Match_CSV_Content):
    ...
    def test_csv_dummy_content_reading_as_class(self):
        dicts_gen = csv_file_to_dicts(path=self.filename1)
...
class TestCSV_Default_DataSource(unittest.TestCase):
    ...
    def test_reading_default_csv(self):
        cr = CSV_Reader(path=self.csv_path)
