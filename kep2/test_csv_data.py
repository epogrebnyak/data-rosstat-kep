...
class Match_CSV_Content(unittest.TestCase):
      ...
      def setUp(self):
          self.csv_content0 = """\tГод / Year\tКварталы / Quarters\t\t\t
Объем ВВП, млрд.рублей /GDP, bln rubles\t\t\t\t\t
2013\t71017\t15892\t17015\t18543\t19567
Комментарий:"""
    
          self.dicts0 = [{ 'head': 'Объем ВВП, млрд.рублей /GDP, bln rubles',
                     'data': ['', '', '', '', ''],  
                    'label': {'unit': '', 'var': ''}},                                        
                   { 'head': '2013',
          ...
          self.rows0 = [['', 'Год / Year', 'Кварталы / Quarters', '', '', ''],
                   ['Объем ВВП, млрд.рублей /GDP, bln rubles', '', '', '', '', ''],
                   ['2013', '71017', '15892', '17015', '18543', '19567'],
                   ['Комментарий:']]
      
      def test_sample_dict_and_rows_lengths(self):
          assert len(self.dicts0) == 3 
          assert len(self.rows0) == 4 
          # skipping a rows with empty 'head'
          assert len(self.dicts0) < len(self.rows0)                  

class TestCSV_yield_from_string(Match_CSV_Content):    
    def test_yield_from_string(self):
        assert list(cd.yield_dicts_from_string(self.csv_content0)) == self.dicts0
...
class TestCSV_Reader(Match_CSV_Content):
    ...
    def setUp(self):
        super().setUp()
        self.filename1 = self.make_file(self.csv_content0)
    ...
    def test_csv_dummy_content_reading_as_class(self):
        ...
        assert cr.rows == self.rows0        
        assert list(cr.yield_dicts()) == self.dicts0
