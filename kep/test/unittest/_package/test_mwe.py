...
'''.lstrip()
    ...
    def check_final_dataframes(self):
        # Note: this function is used twice in this file

        # generate dataframes
        dfa, dfq, dfm = get_dfs()
        # check pandas DataFrame class type
        self.assertTrue(isinstance(dfa, DataFrame))
        self.assertTrue(isinstance(dfq, DataFrame))
        self.assertTrue(isinstance(dfm, DataFrame))
        # check dataframe contents
        self.assertEqual(dfa.to_csv(), self.REF_DFA)
        self.assertEqual(dfq.to_csv(), self.REF_DFQ)
        self.assertEqual(dfm.to_csv(), self.REF_DFM)
    ...
    # run functions get labelled rows form data file and specfile + self.assertEqual(labelled rows are the same
    def test_lab_rows_obtained_from_files(self):
        lab_rows = get_labelled_rows(raw_data_file=self.csvfile(), spec_file=self.specfile(), cfg_file=None)
        self.assertEqual(lab_rows, self.INVESTMENT_PARSED)
        self.file_cleanup()

    # test dataframes obtained from temp data file and temp specfile are equal to refrence dataframes
    def test_dataframes_obtained_from_files(self):
        to_database(raw_data_file=self.csvfile(), spec_file=self.specfile())
        self.check_final_dataframes()
        self.file_cleanup()
